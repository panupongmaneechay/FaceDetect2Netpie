...
key = 't1TxKzOke5ywPak'
secret = 'fEe68DnYgpgBghCnI1VQ8l1RJ'
app = 'WFSsensor'
...
def findTrack(bbox,focus):
    ...
    if pointMove != []:    
        ...
        else : max = ((pow(bbox[2],2) + pow(bbox[3],2))**(1/2))/2
...
while True:
    ...
    Line = (CoorExitLine1[0],CoorExitLine1[1],CoorExitLine2[0] - CoorExitLine1[0],CoorExitLine2[1] - CoorExitLine2[1])
    ...
    for c in cnts :
        ...
        cv2.rectangle(Frame,p1,p2,(colorB,int((indexTrack+1)*70),colorR),2)
        ...
        if abs(firstCenter[1]-CoorExitLine1[1]) > margin and abs(ObjectCentroid[1]-CoorExitLine1[1]) > margin :
            if not CheckLineCrossing(firstCenter, CoorExitLine1, CoorExitLine2) and CheckLineCrossing(ObjectCentroid, CoorExitLine1, CoorExitLine2) :
                ...
                microgear.publish("/countPeople",countPeople,{'retain':True})
